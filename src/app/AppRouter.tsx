    ...
    switch (tabId) {
      ...
      case 'students':
        return {
          ...
          studentNotes,
          onUpdateNote: (name: string, note: string) => {
          ...
          onOpenAttendanceReport: (filter?: string) => {
            setSelectedReportLevel('all');
            setSelectedReportAttendanceFilter((filter as any) || 'all');
      ...
      case 'attendance':
        return {
          ...
          setRecords,
          setExcusedAbsences,
          onOpenClassDaysModal: () => setShowClassDaysModal(true),
          onOpenLiveCheckin: (dayId?: string) => {
            setLiveCheckinDayId(dayId || null);
            setShowLiveCheckinModal(true);
          },
          onOpenReportModal: (filter?: string) => {
            if (filter) setSelectedReportAttendanceFilter(filter as any);
            setShowReportModal(true);
          },
          onOpenMobileDownloadModal: () => setShowMobileDownloadModal(true),
      ...
      case 'schedule':
        return {
          ...
          onTakeAttendanceForDay: (dayId: string) => {
            if (appUser?.role === 'student') return;
            setActiveErpTab('attendance');
            setShowLiveCheckinModal(true);
  ...
  return (
    <>
      ...
      <ApplicationShell
        ...
        syncedBannerMessage={syncedBannerMessage}
        setSyncedBannerMessage={setSyncedBannerMessage}
        pwaInstallable={pwaHook.isInstallable}
        onTriggerPwaInstall={() => pwaHook.triggerInstall()}
        onOpenMobileDownload={() => setShowMobileDownloadModal(true)}
        onPushToCloud={async () => {
          trackUxEvent('sync_retry_requested', { role: appUser?.role || 'guest' });
        ...
        onVerifySupabase={async () => {
          ...
          if (ok) {
            setSupabaseTableMissing(false);
            showToast('success', 'Supabase Connected', 'Database table is online and ready.');
          } else {
            showToast('error', 'Table Not Found', 'Please make sure you ran the SQL query in Supabase.');
          }
        }}
        ...
        onOpenCloudDiagnosticModal={() => setShowDiagnosticModal(true)}
        onOpenExportModal={() => setShowAdminAuditModal(true)}
        onOpenClassDaysModal={() => setShowClassDaysModal(true)}
        onOpenRoleModal={() => setShowRoleMenu(true)}
        onOpenGuideModal={() => setShowGuideModal(true)}
        onOpenNotificationCenter={() => setShowBatchBroadcastModal(true)}
        ...
        setShowFloatingQuizBanner={setShowFloatingQuizBanner}
        showOfflineDrawer={showOfflineDrawer}
        setShowOfflineDrawer={setShowOfflineDrawer}
      >
