  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 overflow-hidden">
      <div className="bg-white border border-slate-200 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden animate-scaleUp">
        ...
        {/* MD3 Modal Header */}
        <div className="px-5 py-4 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-2xl bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-100 dark:border-indigo-800 flex items-center justify-center">
              <Settings className="w-4.5 h-4.5 text-indigo-600" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-sm font-semibold text-slate-800 dark:text-white">Settings</h2>
                {userRole === 'admin' && <span className="md-badge bg-amber-50 text-amber-700 border border-amber-200">Admin</span>}
                {userRole === 'teacher' && <span className="md-badge bg-emerald-50 text-emerald-700 border border-emerald-200">Instructor</span>}
                {isStudent && <span className="md-badge bg-blue-50 text-blue-700 border border-blue-200">Student</span>}
              </div>
              <p className="text-[11px] text-slate-400">Preferences, thresholds & sync</p>
            </div>
          </div>
          <button onClick={onClose} className="md-icon-btn border border-slate-200 dark:border-slate-700">
            <X className="w-4 h-4" />
          </button>
        </div>

        {/* MD3 Tab Row */}
        <div className="flex items-center gap-1 px-4 pt-3 pb-0 border-b border-slate-100 dark:border-slate-800 text-xs font-medium flex-shrink-0 overflow-x-auto">
          <button
            onClick={() => setActiveTab('appearance')}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-t-lg transition-all cursor-pointer border-b-2 -mb-px ${
          ...
          >
            <Sliders className="w-3.5 h-3.5" /> Academic
            {isStudent && <Lock className="w-3 h-3 text-amber-500 ml-1" />}
          </button>
          ...
          >
            <Database className="w-3.5 h-3.5" /> Sync & Backup
            {isStudent && <Lock className="w-3 h-3 text-amber-500 ml-1" />}
          </button>
          ...
          >
            <Info className="w-3.5 h-3.5" /> About HTEIM
          </button>
        </div>
          ...
          {activeTab === 'about' && (
            <div className="space-y-4">
              ...
              <div className="p-4 bg-gradient-to-br from-amber-500/10 via-amber-100/30 to-indigo-50 border border-amber-200 rounded-2xl flex items-center gap-4">
                <LogoImage size="lg" shape="circle" wrapperClassName="flex-shrink-0" />
                <div>
